...
;						"^(?:\:(?P<Nick>[^\!\@ ]+)(?:\!(?P<User>[^\@ ]+))?(?:\@(?P<Host>[^ ]+))? )?(?P<Cmd>[^ ]+)(?: (?P<Params>[^\:][^ ]*(?: [^\:][^ ]*)*))?(?: \:(?P<Msg>.*))?$" ; This is the old regex without tags
		...
		Else {
			If (Params[1] and pNick and pMsg) {
				MessageToLog := Params[1] . " <" . pNick . ">: " . pMsg
				this.Log(MessageToLog) ; This sends only the channel, the name and the message
				}
			Else
				this.Log(Data) ; This sends the whole message received
			}
...
;		this._SendRaw("WHOIS " this.Nick) ; The Twitch API does not support WHO, part of the IRC specification. https://dev.twitch.tv/docs/irc/guide#command--message-limits
	...
	_on353(Tags,Nick,User,Host,Cmd,Params,Msg,Data) {
		...
		for i, Nick in StrSplit(Msg, " ") {
			MODE := []
			Loop, 5 { ; Only loop 5 times, just in case we hang somehow
				if (this.Prefix.Symbols.HasKey(Prefix := SubStr(Nick, 1, 1))) { ; If we can convert the leading symbol into mode letter
					MODE.Insert(this.Prefix.Symbols[Prefix], true) ; Add the mode letter to the mode table,
					Nick := SubStr(Nick, 2) ;  and remove the symbol from the nick
					}
				else
					break
				}
